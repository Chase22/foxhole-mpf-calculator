...
open class AttributeDelegate<T : Any?>(
    ...
    operator fun getValue(
        thisRef: HTMLTag,
        property: KProperty<*>,
    ): T? = thisRef.attributes[key(property)]?.let { deserializer(it) }
...
inline fun <reified T : Enum<T>> enumAttributeDelegate(attributeName: String? = null) =
    AttributeDelegate<T>(attributeName, {
        enumValueOf<T>(it)
    })
